...
#1 /
def index():
    return common.query_handler("""SELECT question.id, question.submission_time, view_number, vote_number, title, message, image, users_id, user_name, reputation FROM question
...
#1 /question/<questionid>/new-comment
    # select_all_users()
...
def insert_tag_to_question(formdata, tagid):    
    common.query_handler("""INSERT INTO question_tag (question_id, tag_id) VALUES(%s, %s)""",
                                 (formdata['question_id'], tagid[0]['id']))

    # tag_by_id(formdata, tagkey)    
    # tagid = common.query_handler('SELECT * FROM tag WHERE name=%s', (formdata['tags'],))
    
    # insert_tag_to_question(formdata, tagid)
    # common.query_handler("""INSERT INTO question_tag (question_id, tag_id) VALUES(%s, %s);""",
                                 # (formdata['question_id'], tagid[0]['id']))
